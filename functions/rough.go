...
func fullwork() {
	...
	input := FieLoader(inputfile)
	splitted := Spliter(input)
	mapped := Mapper(splitted)
	rendered := Render(inputText, mapped)
...
func FieLoader(inputfile string) string {
...
func Render(inputText string, cMap map[rune][]string) string {
